...
public class StreamConsumer {
	@StreamListener(target = Sink.USER_CONSUMER, condition = "headers['contentType'].toString().contains('v1')")
	public void consumeUserV1(Message<UserDtoV1> message) {
		log.info("consumeUserV1: message: {}", message);
	}

	@StreamListener(target = Sink.USER_CONSUMER, condition = "headers['contentType'].toString().contains('v2')")
	public void consumeUserV2(Message<UserDtoV2> message) {
		log.info("consumeUserV2: message: {}", message);
	}

	@StreamListener(target = Sink.ROLE_CONSUMER, condition = "headers['contentType'].toString().contains('v1')")
